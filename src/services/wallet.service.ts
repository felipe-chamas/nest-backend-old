...
export class WalletService {
  ...
  async getWallet(walletId: string) {
    ...
    const {
      data: { result }
    } = await this.httpService.axiosRef.get<GetWalletResult>(`wallets/${walletId}`)
  ...
  async executeMint({ pincode, userId }: WalletBodyDto) {
    ...
    } = await this.httpService.axiosRef.post<MintResult>('transactions/execute', {
      pincode,
      transactionRequest: {
  ...
  async executeUpgrade({ pincode, userId, assetId, value }: PayableNFTWalletBodyDto) {
    // TODO: Change to find by EpicId / SteamId
    const user = await this.userService.findById(userId)
