...
export class SignatureAuthService {
  ...
  verifySolanaSignature(message: string, signature: string, address: string): boolean {
  ...
  verifyEthereumSignature(message: string, signature: string, address: string): boolean {
    // Ref: https://docs.ethers.io/v4/api-utils.html#signatures
    try {
      if (!utils.isHexString(signature, 65)) return false
      const signatureFromMessage = '\x19Ethereum Signed Message:\n' + message.length + message
